...
data = np.load('/Users/estebanmac/Documents/codes/Statistical-mech-phase-simulation/phase1_local/part1_ladder_massive.npz')
...
L = float(data['L'])
N = int(data['N'])
burn_in_frames_raw = int(data['burn_in_frames'])

# Frame skip to keep animation render times reasonable
skip_factor = 2 
trajectory = trajectory[::skip_factor]
T_act = T_act[::skip_factor]
T_targ = T_targ[::skip_factor]
energies = energies[::skip_factor]
order_params = order_params[::skip_factor]
coordinations = coordinations[::skip_factor]

burn_in_frames = burn_in_frames_raw // skip_factor
...
ax_sim.set_title(f'Global Domain (N={N})', fontsize=14, pad=15)

# Define the Zoom Window size (1/3rd of the box)
zoom_w = L / 3
...
# Top Right: Zoomed Physical View (Spans 3 columns)
...
# Connecting lines between rectangle and zoom view
con_bl = ConnectionPatch(xyA=(zoom_min, zoom_min), coordsA=ax_sim.transData,
                         xyB=(zoom_min, zoom_min), coordsB=ax_zoom.transData,
                         color='orange', linestyle='--', linewidth=1.0, alpha=1)
fig.add_artist(con_bl)

con_br = ConnectionPatch(xyA=(zoom_max, zoom_min), coordsA=ax_sim.transData,
                         xyB=(zoom_max, zoom_min), coordsB=ax_zoom.transData,
                         color='orange', linestyle='--', linewidth=1.0, alpha=1)
fig.add_artist(con_br)

con_tl = ConnectionPatch(xyA=(zoom_min, zoom_max), coordsA=ax_sim.transData,
                         xyB=(zoom_min, zoom_max), coordsB=ax_zoom.transData,
                         color='orange', linestyle='--', linewidth=1.0, alpha=1)
fig.add_artist(con_tl)

con_tr = ConnectionPatch(xyA=(zoom_max, zoom_max), coordsA=ax_sim.transData,
                         xyB=(zoom_max, zoom_max), coordsB=ax_zoom.transData,
                         color='orange', linestyle='--', linewidth=1.0, alpha=1)
...
# Bottom Left: Bimodal Density Histogram (Spans 2 columns)
...
# Bottom Center: Energy Trace (Spans 2 columns)
...
ax_temp.set_ylim(t_min * 0.9, t_max * 1.1)
ax_temp.set_ylabel('Temperature', color='red', labelpad=12)

# Bottom Right: Order Parameter (Spans 2 columns) - Moved label to right twin axis to prevent collision
ax_order = fig.add_subplot(gs[1, 4:6])
ax_order.set_xlim(0, num_prod_frames)
ax_order.set_ylim(0, 0.5)
...
ax_order_right.set_ylim(0, 0.5)
ax_order_right.set_ylabel('Largest Droplet Fraction ($m$)', color='purple', labelpad=12)
ax_order_right.set_yticks([]) # Keep ticks hidden on this twin axis to maintain clean data limits

# --- 4. Dynamic Artists ---
scatter = ax_sim.scatter(trajectory[0, :, 0], trajectory[0, :, 1], 
                         s=6, c=coordinations[0], cmap='coolwarm', 
                         vmin=0, vmax=6, alpha=0.8, edgecolors='none')

scatter_zoom = ax_zoom.scatter(trajectory[0, :, 0], trajectory[0, :, 1], 
                               s=25, c=coordinations[0], cmap='coolwarm', 
...
print("Rendering high-resolution MP4 with layout spacing corrections...")
try:
    ani.save('massive_nucleation_dashboard.mp4', writer='ffmpeg', fps=30, dpi=200, bitrate=5000)
    print("Successfully saved high-resolution 'massive_nucleation_dashboard.mp4'!")
except Exception as e:
    print(f"FFmpeg render failed: {e}")
    print("Falling back to GIF...")
    ani.save('massive_nucleation_dashboard.gif', writer='pillow', fps=30, dpi=150)
